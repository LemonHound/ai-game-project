...
test.describe('Checkers Database Integration', () => {
    ...
    test('should store checkers game states when moves are made', async ({
      ...
      const moveResponse = await auth.post('/api/checkers/move', {
        data: {
          ...
          move: {
            from: 40, // Bottom row piece
            to: 33, // Move forward diagonally
            captures: [],
          },
        },
      });
      ...
      expect(oneMove.board_positions).toMatch(/^[RrBb_]{32}$/); // Valid 32-char board format
    });
  ...
  test.describe('Player Game Records', () => {
    test('should create checkers game record on start and complete on finish', async ({
      ...
      // Make several moves to potentially complete a game
      const moves = [
        { from: 40, to: 33, captures: [] },
        { from: 41, to: 34, captures: [] },
  ...
  test.describe('Game Statistics', () => {
    ...
    test('should return popular checkers game states', async ({ request }) => {
      ...
      if (states.length > 0) {
        ...
        expect(firstState.board_positions).toMatch(/^[RrBb_]{32}$/);
